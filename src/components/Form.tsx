    ...
    <div>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div>
          ...
          <select
            className="w-full pl-[20px] pt-[18px] pb-[15px] pr-[18px] mb-[25px] text-sm font-bold tracking-tight"
            {...register("paymentTerms")}
          >
            {paymentTerms.map((term, index) => (
              <option key={`term-${index}`} value={term}>{`Net ${term} ${
                term === 1 ? "day" : "days"
              }`}</option>
            ))}
          </select>
        ...
        />
        <p className="mt-[69px] text-[#777F98] mb-[22px] text-[23px] font-bold">
          Item List
        </p>
        {items.map((item, i) => (
          <div>
            <div className="flex flex-col">
              <label className="text-[#7E88C3] text-sm mb-[15px]">
                Item Name
              </label>
              <input
                className="pl-[20px] pt-[18px] pb-[15px] pr-[18px] mb-[25px] text-sm font-bold tracking-tight"
                type="text"
                name="name"
                value={item.name}
            ...
            <div className="flex items-center">
              <div className="w-[64px] mr-4">
                <label>Qty</label>
                <input
                  className="w-full pl-[20px] pt-[18px] pb-[15px] pr-[18px]"
                  type="text"
                  name="quantity"
                  value={item.quantity}
                  onChange={(e) => handleChange(e, i)}
